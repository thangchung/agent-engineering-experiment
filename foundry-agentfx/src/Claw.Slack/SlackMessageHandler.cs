    ...
    : IEventHandler<MessageEvent>, IEventHandler<AppMention>
{
    ...
    {
        using var activity = ActivitySource.StartActivity("slack.dm", ActivityKind.Consumer);
    ...
    {
        using var activity = ActivitySource.StartActivity("slack.mention", ActivityKind.Consumer);
        activity?.SetTag("channel.type", "slack");
...
public static class SlackChannelExtensions
{
    ...
    {
        var botToken = config["Slack:BotToken"]
            ?? throw new InvalidOperationException("Slack:BotToken is not configured.");
        var appToken = config["Slack:AppToken"]
            ?? throw new InvalidOperationException("Slack:AppToken (xapp-...) is not configured.");
    ...
    public static WebApplication MapSlack(this WebApplication app)
    {
