...
public sealed class OpenSandboxRunner : ISandboxRunner
{
        ...
        CancellationToken ct)
    {
        ...
        try
        {
            execution = await sandbox.Commands.RunAsync(command, cancellationToken: ct).WaitAsync(ct);
    ...
    private sealed record SandboxExecutionPayload(bool Ok, object? FinalValue, string? Error, string? Traceback, string? Stdout, string? Stderr);
}
